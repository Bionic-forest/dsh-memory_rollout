...
> Early version (0.1.x), 51/51 tests passing, in candidate-release observation. Feedback is welcome.
...
> Version `0.1.9`. The plugin is **not yet published to the npm registry** (`dsh plugin add dsh-memory_rollout` needs the package to be published first), so the **redeemable** install path is the **GitHub repo / local tgz**.
