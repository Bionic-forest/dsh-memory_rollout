...
// 覆盖三种情形：
//  1. 无变化：.stage1-state.json 无增量（outputs 的 source_watermark == lastSuccessWatermark）
...
try {
  ...
  console.log('[2] 有增量 + LLM 正常：发布 + ran:true ok:true + 水印推进')
  {
    writeState({
      jobs: {},
      outputs: { 'j-1': { source_watermark: 'wm2', session_id: 's1', rollout_summary: 'new summary', rollout_slug: 'slug1', keywords: 'a,b', generated_at: '2026-01-02T00:00:00.000Z' } },
    ...
    const st = readState()
    check(st.global.lastSuccessWatermark === 'wm2', 'lastSuccessWatermark advanced to wm2')
  ...
  console.log('[3] 有增量 + LLM 输出含秘密：校验失败 → 不发布 + ok:false + errors')
  {
    ...
    writeState({
      jobs: {},
      outputs: { 'j-2': { source_watermark: 'wm3', session_id: 's2', rollout_summary: 'secret-y', generated_at: '2026-01-03T00:00:00.000Z' } },
      global: { lastSuccessWatermark: 'wm2', lastPhase2At: '' },
    })
