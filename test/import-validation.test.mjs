...
const bundle = (files, entries = []) => JSON.stringify({ format: 'dsh-memory_rollout-memory-backup', version: 1, files, entries })

const tmp = path.join(os.tmpdir(), 'dsh-memory_rollout-importval-' + Date.now())
