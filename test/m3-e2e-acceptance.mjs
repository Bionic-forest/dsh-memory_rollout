...
//
// 用法：cd plugins/dsh-memory_rollout && node test/m3-e2e-acceptance.mjs  （安全：纯隔离 temp home）
...
const tempHome = path.join(os.tmpdir(), 'dsh-memory_rollout-m3-e2e-' + Date.now())
...
// ── Phase A2：M3 管理页 /dsh-memory_rollout/overview 返回 status 字段（真实 handler）──
console.log('[A2] /dsh-memory_rollout/overview 返回 status 字段（M3 管理页“至少回答”6 项）')
{
  const overview = registeredRoutes.find((r) => r.path === '/dsh-memory_rollout/overview')
  check(!!overview, '捕获到 /dsh-memory_rollout/overview 路由')
  let body = null
  const res = {
    statusCode: 0,
    setHeader() {},
