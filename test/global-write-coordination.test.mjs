...
const importBundle = JSON.stringify({ format: 'dsh-memory_rollout-memory-backup', version: 1, files: [{ path: 'rollout_summaries/x.md', content: b64('x') }], entries: [{ id: 'e1', content: 'x', createdAt: '2026-08-27T00:00:00.000Z' }] })

const tmp = path.join(os.tmpdir(), 'dsh-memory_rollout-writecoord-' + Date.now())
...
try {
  await apply(ctx, {})
  assert.ok(routes['/dsh-memory_rollout/import'] && routes['/dsh-memory_rollout/entries'], 'routes registered')
  ...
  console.log('[1] import in flight rejects a concurrent UI write')
  {
    ...
    const pA = post('/dsh-memory_rollout/import', importBundle)
    await new Promise((r) => setTimeout(r, 25)) // let the import acquire writeBusy + hit its slow put
    const rUI = await post('/dsh-memory_rollout/entries', JSON.stringify({ action: 'add', content: 'during import' }))
  ...
  {
    table.putDelayMs(0)
    const r = await post('/dsh-memory_rollout/entries', JSON.stringify({ action: 'add', content: 'after import' }))
