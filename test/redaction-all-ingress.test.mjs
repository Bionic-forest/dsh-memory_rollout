...
const tmp = path.join(os.tmpdir(), 'dsh-rollout-ingress-' + Date.now())
process.env.DSH_HOME = tmp
fs.mkdirSync(tmp, { recursive: true })

try {
  await apply(ctx, {})
  assert.ok(routes['/dsh-rollout/entries'], 'entries route registered')
  ...
  {
    const r = await call('/dsh-rollout/entries', JSON.stringify({ action: 'add', content: SECRET, tags: ['a', 'b'] }))
  ...
  console.log('[2] import entry redacts the restored table content')
  {
    ...
    const r = await call('/dsh-rollout/import', JSON.stringify(bundle))
  ...
  {
    const r = await call('/dsh-rollout/entries', JSON.stringify({ action: 'add', content: 'set up the build config and run the tests', tags: [] }))
    const raw = table._m.get(r.body.id)
