// dsh-rollout — cross-session memory vault for DeepSeek Harness (Client half).
//
...
// (GET/POST /dsh-rollout/entries), and adds a Settings block that:
//   - reads + edits the plugin config (GET/POST /dsh-rollout/config),
//   - exports the whole memories/ tree for backup (GET /dsh-rollout/export),
//   - imports a backup file (POST /dsh-rollout/import).
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const apiEntries = (method, payload) => api('/dsh-rollout/entries', method, payload)
    ...
    function MemoryPage() {
      ...
      const saveConfig = () => {
        ...
        setCfgSaving(true)
        setStatus('')
        api('/dsh-rollout/config', 'POST', payload)
          .then((res) => {
      ...
      const onImportFile = (e) => {
        ...
        reader.onload = () => {
          api('/dsh-rollout/import', 'POST', String(reader.result || ''))
            .then((res) => {
              setStatus(
                '导入完成：' +
                  (res.fileCount || 0) +
      ...
      return React.createElement(
        ...
        React.createElement('div', { style: { fontWeight: 600 } }, '记忆库（dsh-rollout 跨会话记忆）'),
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              'a',
              { href: '/dsh-rollout/export', download: true, style: { flexShrink: 0 } },
              '导出记忆',
            ),
            React.createElement(
              'label',
    ...
    function apply(ctx) {
      ...
      slots.inject('settings.section', () =>
        slots.register(
          { name: 'settings.section', id: 'dsh-rollout', order: 30, label: () => '记忆库' },
