// dsh-memory-rollout — cross-session memory vault for DeepSeek Harness (Client half).
//
...
// (GET/POST /dsh-memory-rollout/entries), and adds a Settings block that:
//   - reads + edits the plugin config (GET/POST /dsh-memory-rollout/config),
//   - exports the whole memories/ tree for backup (GET /dsh-memory-rollout/export),
//   - imports a backup file (POST /dsh-memory-rollout/import).
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const apiEntries = (method, payload) => api('/dsh-memory-rollout/entries', method, payload)
    ...
    function MemoryPage() {
      ...
      const loadConfig = () => {
        api('/dsh-memory-rollout/config', 'GET')
          .then((res) => {
      ...
      const saveConfig = () => {
        ...
        setCfgSaving(true)
        setStatus('')
        api('/dsh-memory-rollout/config', 'POST', payload)
          .then((res) => {
      ...
      const onImportFile = (e) => {
        ...
        reader.onload = () => {
          api('/dsh-memory-rollout/import', 'POST', String(reader.result || ''))
            .then((res) => {
              setStatus(
                '导入完成：' +
                  (res.fileCount || 0) +
      ...
      return React.createElement(
        ...
        React.createElement('div', { style: { fontWeight: 600 } }, '记忆库（dsh-memory-rollout 跨会话记忆）'),
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              'a',
              { href: '/dsh-memory-rollout/export', download: true, style: { flexShrink: 0 } },
              '导出记忆',
            ),
            React.createElement(
              'label',
    ...
    function apply(ctx) {
      ...
      slots.inject('settings.section', () =>
        slots.register(
          { name: 'settings.section', id: 'dsh-memory-rollout', order: 30, label: () => '记忆库' },
