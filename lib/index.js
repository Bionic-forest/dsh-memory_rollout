...
export async function apply(ctx, config) {
  ...
  async function upsertStage1Session(sid, { active = false, summarized = false, status } = {}) {
    ...
    const rec = await stage1MetaTable.update(STAGE1_META_KEY, (m) => {
      // GPT P0-7：不可变返回新对象（含嵌套 sessions），绝不原地改传入对象，
      // 否则后端写失败时内存旧值已被提前改掉、与磁盘分叉。
      ...
      return { ...(m || {}), sessions: { ...sessions, [sid]: next } }
    })
  ...
  async function reconcileChangeOutbox() {
    ...
    for (const [id, e] of Array.from(table.entries())) {
      ...
      const referenced = Array.from(memoryChangesTable.entries()).some(([, c]) => {
        if (!c || typeof c.payload !== 'object') return false
        return c.payload.entryId === id || c.payload.targetId === id
      })
    ...
    return withWrite(async () => {
      ...
      for (const w of missing) {
        const dup = Array.from(memoryChangesTable.entries()).some(([, c]) =>
          c && typeof c.payload === 'object' && (c.payload.entryId === w.payload.entryId || c.payload.targetId === w.payload.targetId))
        if (dup) continue
  ...
  /**
   * GPT P0-4 恢复：解除「指向不存在/终态失败批次」的 input/change 绑定，允许重新选择；
   * 同时把 batches 的 input_ids 里未绑定的记录补绑（幂等），消除跨 key 半提交孤儿。
   */
  async function reconcilePhase2Bindings(nowMs) {
    return withWrite(async () => {
      const jobStatus = new Map()
      for (const [id, j] of phase2JobsTable.entries()) if (j) jobStatus.set(id, j.status)
      let fixed = 0
      const unbindOrphan = async (table, key, rec) => {
        if (!rec || !rec.phase2_batch_id) return
        const target = jobStatus.get(rec.phase2_batch_id)
        const orphan = !target || target === 'failed_terminal'
        if (orphan) {
