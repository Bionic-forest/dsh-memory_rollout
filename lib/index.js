...
export function clampPromptInputs(inputs, currentSummary, currentRegistry, caps = {}) {
  ...
  const maxCur =
    Number(caps.currentSummaryChars) > 0 ? Number(caps.currentSummaryChars) : PROMPT_CURRENT_SUMMARY_CHARS
  const maxReg =
    Number(caps.currentRegistryChars) > 0 ? Number(caps.currentRegistryChars) : PROMPT_CURRENT_REGISTRY_CHARS
  const list = Array.isArray(inputs) ? inputs : []
  const kept = list.slice(0, maxIn).map((it) => ({
    ...it,
    rollout_summary: clampChars(it && it.rollout_summary, perIn),
  }))
  return {
    inputs: kept,
    currentSummary: clampChars(currentSummary, maxCur),
    currentRegistry: clampChars(currentRegistry, maxReg),
...
export async function apply(ctx, config) {
  ...
  function scheduleStage1Drain() {
    ...
    setImmediate(() => {
      ...
      drainStage1Jobs().catch((err) => {
        try { console.error('[dsh-memory_rollout] stage-1 drain error:', err) } catch {}
      })
    })
  }
  function scheduleStage1Wake(nextAt) {
    ...
    stage1WakeTimer = setTimeout(() => {
      stage1WakeTimer = null
      drainStage1Jobs().catch((err) => {
        try { console.error('[dsh-memory_rollout] stage-1 wake drain error:', err) } catch {}
      })
    }, delay)
  }
  ...
  function buildConsolidationPrompt(inputs, currentSummary, currentRegistry, changes, opts = {}) {
    ...
    const clad = clampPromptInputs(inputs, currentSummary, currentRegistry, {
      ...
      currentSummaryChars: PROMPT_CURRENT_SUMMARY_CHARS,
      currentRegistryChars: PROMPT_CURRENT_REGISTRY_CHARS,
    })
  ...
  async function enqueueCompressBatch(reason) {
    if (!currentSummaryOverCap()) return { enqueued: false, reason: 'not-over-cap' }
  ...
  function requestPhase2Integrate() {
    ...
    phase2AutoTimer = setImmediate(() => {
      ...
      phase2Integrate().catch((err) => {
        try { console.error('[dsh-memory_rollout] phase-2 auto integrate error:', err) } catch {}
      })
    })
   ...
   *  P0-9：已到期的非终态批（av<=nowMs 或 le<=nowMs 或缺失）返回「立即」（nowMs），而非 null——
   *  否则到期附近若发生再次调度/忙/竞态重排，过期批会从时间调度视野消失，无新事件时永久停住（延迟重试失醒）。
   *  立即唤醒由 phase2Integrate 的单飞（phase2Busy）吸收；已领取批的 retry_wait 退避总是未来 available_at（≥30s），不形成忙循环。 */
  function nextPhase2WakeAt(nowMs) {
    ...
    for (const [, j] of phase2JobsTable.entries()) {
      ...
      if (j.status === 'retry_wait' || j.status === 'pending') {
        // 已到期或缺失可用时间 → 立即唤醒；否则等到 available_at。
        if (!av || av <= nowMs) next = Math.min(next, nowMs)
        else next = Math.min(next, av)
      } else if (j.status === 'running' || j.status === 'prepared' || j.status === 'published') {
        // 租约已到期/缺失 → 立即唤醒（recover 会重做/补提交）；否则等租约到期。
        if (!le || le <= nowMs) next = Math.min(next, nowMs)
        else next = Math.min(next, le)
      }
    }
    return Number.isFinite(next) ? next : null
  }
  ...
  function schedulePhase2Wake(nextAt) {
    ...
    phase2WakeTimer = setTimeout(() => {
      phase2WakeTimer = null
      phase2Integrate().catch((err) => {
        try { console.error('[dsh-memory_rollout] phase-2 wake drain error:', err) } catch {}
      })
    }, delay)
  }
