...
export async function apply(ctx, config) {
  ...
  async function archiveVault(opts = {}) {
    ...
    const report = { dryRun, archived: 0, candidates: { stage1_jobs: 0, stage1_outputs: 0, phase2_jobs: 0, changes: 0 } }
    return withWrite(async () => {
  ...
  const archiveVaultTool = defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [
        { type: 'text', text: `归档${value.dryRun ? '(dry-run)' : ''}：已归档 ${value.archived}；候选 jobs=${value.candidates.stage1_jobs} outputs=${value.candidates.stage1_outputs} phase2=${value.candidates.phase2_jobs} changes=${value.candidates.changes}` },
      ],
    },
    async execute(args) {
