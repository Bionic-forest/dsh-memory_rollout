...
export const Config = z.object({
  ...
  /** Max pipeline runs per calendar day. 【孤儿/仅兼容】：新管线（Stage1 入队 + drain + 每日
   * 模型尝试额度 maxModelAttemptsPerDay）不再消费本配置，保留仅因旧设置页 / 旧文档仍引用，
   * 无运行时消费。 */
...
const CONFIG_FIELDS = [
  ...
  { key: 'maxPipelineRunsPerDay', label: '每日管线运行上限（maxPipelineRunsPerDay）【孤儿/仅兼容】', type: 'number', hint: '【孤儿】仅兼容旧设置页 / 旧文档，新管线已无运行时消费（每日额度由 maxModelAttemptsPerDay 承担）。默认 12。' },
...
const recordSchema = zod.object({
  ...
  // 阶段 C（§10.1/§10.4）：生命周期字段。旧记录没有这些字段，读取时由
  // allEntries() 补默认值，写入时由 zod default 补；显式写操作（memory_remember）
  // 不填它们（默认即可），只有当召回/忘却被记入 usage 时才更新 last_used_at/usage_count。
  // status：active | superseded | forgotten（P1-4 生命周期谓词）；superseded_by 记录
  // 替代其事实的条目 id（可追替代事实）。二者都由生命周期写操作设置，普通 remember 默认 active/''。
  last_used_at: zod.string().default(''),
...
// supersede/import 次之，remember/note/draft 常规。Phase2 取 pending 变更构造提示词。
...
/**
 * 阶段 C：记忆新鲜度（§10.1）。基于 entry 的 status 与最近使用/更新时间判定。
...
export function freshnessOf(entry, now = Date.now()) {
  ...
  const last = entry.last_used_at || entry.updatedAt || ''
...
/**
 * 阶段 C：召回排序分（§10.4 / 相关性+新鲜度+使用反馈）。
 * `o.relevance`(0..1) 由调用方计算，`o.freshness`(1/0.5/0) 由 freshnessOf 映射，
 * usage_count 归一化。返回 0..1 分数。
 */
export function scoreMemory(entry, o = {}) {
  const relevance = Number(o.relevance) || 0
  const freshness = Number(o.freshness) || 0
  const usage = Math.min(1, (Number(entry && entry.usage_count) || 0) / 5)
  return relevance * 0.6 + freshness * 0.3 + usage * 0.1
}
...
export async function apply(ctx, config) {
  ...
  const readStage1Meta = () => {
    ...
    return m && typeof m === 'object'
      ? m
      : { runDay: '', modelAttemptsToday: 0, lastSuccessWatermark: '', lastPhase2At: '', phase2_last_error: '', sessions: {} }
  }
  const writeStage1Meta = (patch) => {
    const cur = readStage1Meta()
    return stage1MetaTable.put(STAGE1_META_KEY, { ...cur, ...patch })
  }
  // 逐会话活动水位（取代旧 .pipeline-state.json 的 sessions。§11.4⑥③ / R7）。
  // 旧管线靠它做「次级候选 idle-time / hasNewActivitySinceSummary」判断；新管线（Stage1 入队
  // + drain 只消费被入队的会话）不再用它选择候选，但仍保留它作为活动/汇总水位，供
  // turn/end 记录活动、memory_precompact 标「该会话已手动落盘」。
  const readStage1Sessions = () => {
    const m = readStage1Meta()
    return m.sessions && typeof m.sessions === 'object' ? m.sessions : {}
  }
  const readStage1SessionRec = (sid) => {
    const sessions = readStage1Sessions()
    return sessions[sid] || { sessionId: String(sid), lastActivityAt: '', summarizedAt: '', lastExtractStatus: '' }
  }
  async function upsertStage1Session(sid, { active = false, summarized = false, status } = {}) {
    // 活动水位（sessions）是跨「多个异步入口」共享写：用 storage-domain 的
    // update('meta', fn)（唯一真原子读改写）单记录更新，避免 read-modify-write 整对象
    // 的 lost update。先保证 meta key 存在（存储域 update 要求 key 存在，缺则 put 默认）。
    if (!stage1MetaTable.get(STAGE1_META_KEY)) {
      await stage1MetaTable.put(STAGE1_META_KEY, {
        runDay: '', modelAttemptsToday: 0, lastSuccessWatermark: '',
        lastPhase2At: '', phase2_last_error: '', sessions: {},
      })
    }
    const rec = await stage1MetaTable.update(STAGE1_META_KEY, (m) => {
      const sessions = (m && m.sessions && typeof m.sessions === 'object') ? m.sessions : {}
      const cur = sessions[sid] || { sessionId: String(sid), lastActivityAt: '', summarizedAt: '', lastExtractStatus: '' }
      const next = { ...cur }
      if (active) next.lastActivityAt = nowIso()
      if (summarized) next.summarizedAt = nowIso()
      if (status !== undefined) next.lastExtractStatus = status
      sessions[sid] = next
      m.sessions = sessions
      return m
    })
    return rec.sessions[sid]
  }

  // ── 统一变更流（R5 / P1-2 / §9）：所有手动记忆入口写 memory_changes ───────────
  // priority：forget 最高（墓碑强语义，内容即使新增也绝不进权威摘要/召回），supersede/import
  // 次之（取代关系/导入内容随批进权威），remember/note/draft 常规。Phase2 按 priority 加权
  // （forget 最高优先）构件提示词与排除规则。
  const CHANGE_PRIORITY = { forget: 100, supersede: 90, import: 80, remember: 10, note: 10, draft: 10 }
  ...
  /**
   * 阶段 C：记录一次召回/使用反馈（§10.4 usage feedback）。把 `last_used_at` 设为 now、
   * `usage_count` +1，并写回表。返回是否更新成功。由调用方决定是否包 withWrite；
   * 内部不调用任何加锁函数（防嵌套死锁）。
   */
  async function recordUsage(id, _kind) {
    for (const [key, value] of Array.from(table.entries())) {
      if (key === id) {
        await table.put(id, {
          ...value,
          last_used_at: nowIso(),
          usage_count: (Number(value && value.usage_count) || 0) + 1,
        })
        return true
      }
    }
    ...
    return withWrite(async () => {
      ...
      for (const k of toReclaim) {
        await stage1JobsTable.update(k, (cur) => {
          cur.status = 'pending'
          cur.lease_owner = ''
          cur.lease_expires_at = ''
          cur.updated_at = new Date(nowMs).toISOString()
          return cur
        })
        reclaimed++
      }
  ...
  async function claimNextStage1Job(nowMs) {
    return withWrite(async () => {
      ...
      // 用 update(key,fn) 在写链槽位原子置 running + 租约（fields 级 read-modify-write）。
      await stage1JobsTable.update(key, (cur) => {
        cur.status = 'running'
        cur.lease_owner = bootId
        cur.lease_expires_at = new Date(nowMs + STAGE1_LEASE_MS).toISOString()
        cur.updated_at = new Date(nowMs).toISOString()
        return cur
      })
      return pick
    ...
    return withWrite(async () => {
      ...
      const job = await stage1JobsTable.update(key, (cur) => {
        ...
        if (status === 'succeeded_with_output' && extraction) {
          cur.status = 'succeeded_with_output'
          cur.completed_at = now.toISOString()
          ...
          cur.last_error = errMsg || ''
          cur.last_error_code = ''
          cur.last_error_message = errMsg || ''
          const maxAttempts = cur.max_attempts || 3
          if (cur.attempt_count >= maxAttempts) {
            cur.status = 'failed_terminal'
            cur.completed_at = now.toISOString()
          } else {
            cur.available_at = new Date(now.getTime() + stage1BackoffSeconds(cur.attempt_count) * 1000).toISOString()
          }
        }
        return cur
      })
      ...
      if (status === 'succeeded_with_output' && extraction) {
        ...
        const evidence = buildEvidenceContent(job.session_id, cwd || '', extraction)
        ...
        await stage1OutputsTable.put(stage1OutputKey(job.id), {
          job_id: job.id,
          session_id: job.session_id,
          source_watermark: job.source_watermark,
  ...
  async function drainStage1Jobs() {
    ...
    // 每次 drain 先回收过期租约（§5：时间驱动，非启动一次性）。
  ...
  function scheduleStage1Wake(nextAt) {
    ...
    // 单次最长 60s，避免超长 sleep 在进程退出/重启时悬挂。
    const delay = Math.max(0, Math.min(60000, nextAt - Date.now()))
  ...
  /** §4 事件入队（不丢）：withWrite 忙时「有限重试 + 最终明确日志」，绝不静默丢。 */
  async function enqueueStage1JobIntoTable(sessionId, watermark, opts = {}) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      try {
        return await withWrite(async () => {
          const existing = stage1JobsTable.get(key)
          if (existing) return { queued: false, key, job: existing }
          const now = new Date()
          const job = {
            id: 'j-' + now.getTime().toString(36) + '-' + Math.random().toString(36).slice(2, 6),
            session_id: String(sessionId),
        ...
        lastErr = err
        await wait(50 + attempt * 25)
      }
    }
    // 锁忙重试耗尽：绝不静默丢——记录明确报错日志（§4 P0-2）。
    // 【已知限制 · 选 b】withWrite 持锁窗很短（仅单条表读改写，无跨进程/长 IO），
    // 8 次重试（退避 ~1.3s）后仍拿不到锁的概率极低；此路径不建持久收件箱兜底。
    // 落单的会话由幂等 watermark 去重在下一次触发 / memory_precompact / Phase2 wake
    // 时重新入队覆盖（事件触发是幂等的），不丢作业、不造成双份。若未来写锁窗显著变长
    // （如引入跨目录大 IO），应改为持久收件箱（memory_changes kind=extract pending）由
    // drain 补办。当前风险低，明确标注为已知限制，不新建收件箱。
    try { console.error('[dsh-rollout] enqueue dropped after retries — no durable inbox; risk low because write lock window is short:', lastErr) } catch {}
    return { queued: false, key, error: String((lastErr && lastErr.message) || lastErr) }
  }
   ...
   */
  async function commitPhase2Batch(batch, nowMs) {
    ...
    return withWrite(async () => {
      ...
      for (const oid of inputIds) {
        ...
        await stage1OutputsTable.update(oid, (curO) => {
          curO.phase2_batch_id = id
          curO.selected_for_phase2 = true
          return curO
        })
      }
      ...
      for (const cid of changeIds) {
        ...
        await memoryChangesTable.update(cid, (curC) => {
          curC.status = 'consumed'
          curC.phase2_batch_id = id
          return curC
        })
      }
      const m = readStage1Meta()
      ...
      await phase2JobsTable.update(id, (curJ) => {
        curJ.status = 'committed'
        curJ.updated_at = now.toISOString()
        return curJ
      })
  ...
  async function failPhase2Batch(batch, errMsg, nowMs, errors) {
    ...
    await withWrite(async () => {
      ...
      await phase2JobsTable.update(batch.id, (cur) => {
        cur.attempt_count = attempt
        cur.last_error = errMsg
        cur.updated_at = now.toISOString()
        if (terminal) {
          cur.status = 'failed_terminal'
        } else {
          cur.status = 'retry_wait'
          cur.available_at = new Date(nowMs + phase2BackoffSeconds(attempt) * 1000).toISOString()
        }
        return cur
      })
    })
      ...
      if (job.status === 'published') {
        ...
        if (expired || foreign) {
          await phase2JobsTable.update(id, (cur) => {
            cur.attempt_count = (cur.attempt_count || 0) + 1
            cur.status = 'retry_wait'
            cur.available_at = new Date(nowMs + phase2BackoffSeconds(cur.attempt_count) * 1000).toISOString()
            cur.updated_at = new Date(nowMs).toISOString()
            return cur
          })
   ...
   * phase2_batch_id 防重复领取。返回被领取的 job，或 null。
   */
  async function claimNextPhase2Job(nowMs) {
    return withWrite(async () => {
      for (const [id, job] of phase2JobsTable.entries()) {
        ...
        if (duePending || dueRetry) {
          await phase2JobsTable.update(id, (cur) => {
            cur.status = 'running'
            cur.lease_owner = bootId
            cur.lease_expires_at = new Date(nowMs + PHASE2_LEASE_MS).toISOString()
            cur.updated_at = new Date(nowMs).toISOString()
            return cur
          })
          ...
          return { ...job, id, status: 'running', lease_owner: bootId, lease_expires_at: new Date(nowMs + PHASE2_LEASE_MS).toISOString() }
        }
      }
      const unconsumed = []
      ...
      for (const oid of unconsumed) {
        await stage1OutputsTable.update(oid, (curO) => {
          curO.phase2_batch_id = batchId
          return curO
        })
      }
      for (const cid of pendingChanges) {
        await memoryChangesTable.update(cid, (curC) => {
          curC.phase2_batch_id = batchId
          return curC
        })
      }
  ...
  async function processPhase2Batch(batch) {
    ...
    // 慢速模型调用在写锁外执行（M4）。
    let result = await consolidateWithLlm(prompt)
    if (!result) {
    ...
    const commitRes = await commitPhase2Batch(batch, nowMs)
    return {
      ran: true,
      ok: true,
  ...
  async function phase2Integrate() {
    ...
    // 2) 领取一个批次（重试优先，否则冻结新批）。
    const claimed = await claimNextPhase2Job(now)
    if (!claimed) {
      schedulePhase2Wake(nextPhase2WakeAt(Date.now()))
      return { ran: false, reason: 'no-change' }
    }
    // 3) 处理（LLM → 校验 → staging → publish → commit）。
    const result = await processPhase2Batch(claimed)
    schedulePhase2Wake(nextPhase2WakeAt(Date.now()))
    return result
  }
  ...
  const rememberTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      return withWrite(async () => {
        ...
        // 更新其 usage 并返回既有 id（不产生重复事实）。仍处理显式 supersedes（若给出）。
        for (const e of allEntries()) {
          if (
            e.status === 'active' &&
            ...
            await table.put(e.id, {
              ...cur,
              last_used_at: nowIso(),
              usage_count: (Number(cur && cur.usage_count) || 0) + 1,
              updatedAt: nowIso(),
            })
  ...
  function sourceRefForEntry(e) {
    ...
    for (const [, output] of stage1OutputsTable.entries()) {
    ...
    return foundBroken ? { broken: true } : null
  }
  ...
  const recallTool = defineTool({
    ...
    async execute(args) {
      ...
      // 阶段 C（M6）：召回排序纳入相关性 + 新鲜度 + 使用反馈（§10.4）。
      // `relevance` = scoreEntry 原始分（相关性），`freshness` = freshnessOf 映射
      // （fresh=1/aging=0.5/其余=0），usage_count 由 scoreMemory 内部归一化。
      ...
      // 阶段 C（M6）：对本次被召回的条目记录使用反馈（last_used_at=now、usage_count+1）。
      // 在 withWrite 内、但 best-effort：并发写（如 import）持锁时召回依然返回结果——
      // 使用反馈是元数据，不该让一次「读」因锁冲突而失败。
      try {
        await withWrite(async () => {
          for (const e of top) await recordUsage(e.id, 'recalled')
        })
      } catch {
        // best-effort: a concurrent write should not break a recall.
      }
  ...
  const draftTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      // L8：草稿写文件包 withWrite（writeSessionDraft 内部只做 ensureLayout+writeText，
      // 不调用任何加锁函数，不会嵌套死锁）。R5：草稿内容同时入统一变更流（带 source_ref），
      // 随下一批进权威版本作为来源/证据。
      const file = await withWrite(async () => {
        const f = writeSessionDraft(sid, cwd, args.title, body)
        // R5：草稿内容（脱敏）同时入统一变更流（带 source_ref），随下一批进权威版本作为来源/证据。
        await writeChangeRecord('draft', {
          content: redactSecrets(body),
          title: redactSecrets(String(args.title || '')),
          sessionId: sid,
          file: f,
        }, { source_ref: f })
        return f
  ...
  const precompactTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      // ② 活动/汇总水位写 stage1_meta（R7：不再写旧 .pipeline-state.json）。
      await upsertStage1Session(sid, { active: true, summarized: true })
  ...
  // ── P1-3 / R7：一次性迁移旧 .pipeline-state.json（退役旧管线）──────────────
  // 旧管线的逐会话活动/汇总水位（sessions）压平进 stage1_meta.sessions；旧文件归档
  // 为只读 .bak-pstate-<ts>，随后不再依赖（不再读写 .pipeline-state.json）。
  const legacyPipelineStatePath = () => path.join(memoryRoot(), '.pipeline-state.json')
  const readLegacyPipelineState = () => {
    try {
      const p = JSON.parse(readText(legacyPipelineStatePath()))
      return p && typeof p === 'object' ? p : {}
    } catch {
      return {}
    }
  }
  const migratePipelineStateFromFile = async () => {
    try {
      if (!fs.existsSync(legacyPipelineStatePath())) return 0
      const legacy = readLegacyPipelineState()
      let n = 0
      await withWrite(async () => {
        const sessions = readStage1Sessions()
        for (const [sid, rec] of Object.entries(legacy.sessions || {})) {
          if (!rec || !sessions[sid]) {
            sessions[sid] = {
              sessionId: String(sid),
              lastActivityAt: (rec && rec.lastActivityAt) || '',
              summarizedAt: (rec && rec.summarizedAt) || '',
              lastExtractStatus: (rec && rec.lastExtractStatus) || '',
            }
            n++
          }
        }
        await writeStage1Meta({ sessions })
      })
      const archive = legacyPipelineStatePath() + '.bak-pstate-' + Date.now()
      fs.renameSync(legacyPipelineStatePath(), archive)
      try { console.error(`[dsh-rollout] pipeline-state migration: flattened ${n} session records into stage1_meta, archived old file -> ${archive}`) } catch {}
      return n
    } catch (err) {
  ...
  try {
    await migratePipelineStateFromFile()
  } catch (err) {
  ...
  const eventDisposers = [
    ...
    ctx.on('session/event', async (session, event) => {
      ...
      try {
        // 逐会话活动水位写 stage1_meta（R7：不再写旧 .pipeline-state.json）。
        if (type === 'turn/end') {
          const sid = session && session.id ? String(session.id) : ''
          if (sid && config.autoTrigger !== 'off') {
            // 活动水位为非关键簿记：fire-and-forget，避免逐 turn 回调被写锁拖慢。
            upsertStage1Session(sid, { active: true }).catch(() => {})
          }
          return
        }
        ...
        if (type === 'compaction/start' && config.precompactAuto && config.autoTrigger !== 'off') {
          ...
          if (sid) {
            ...
            upsertStage1Session(sid, { active: true, summarized: true }).catch(() => {})
          }
          scheduleStage1Drain()
          schedulePhase2Wake(nextPhase2WakeAt(Date.now()))
        }
