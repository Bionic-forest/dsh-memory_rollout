...
export async function apply(ctx, config) {
  ...
  // 使「新会话能想起过去自动形成的偏好/决定/项目状态」。仅关键词 + 注册表行范围（M1 停止条件：
  // 不引入 embedding/向量库/全盘模糊搜索）。返回 { content, citation }（引用 = 文件:start-end）。
  function searchMemoryFiles(terms, limit) {
    ...
    for (const f of files) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const hits = terms.filter((term) => tl.includes(term)).length
        if (hits <= 0) continue
        out.push({ content: t, citation: `${f.label}:${i + 1}-${i + 1}`, m: hits })
      }
    }
