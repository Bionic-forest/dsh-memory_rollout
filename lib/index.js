...
export async function apply(ctx, config) {
    ...
    try {
      ...
      for (;;) {
        ...
        try {
          ...
          //  - persisted === null：sessionQuery 插件缺失（部署能力问题，非该会话源缺失）——按空源
          //    no-op，避免无 sessionQuery 的部署永远 retry（与 drain-stage1 契约一致）。
  ...
  const rememberTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      // P0：记住产生 pending memory_change → 请求 Phase2 整合（idle 即自动进 current）。
      requestPhase2Integrate()
      return res
    },
  })
        ...
        try {
          ...
          if (req.method === 'POST') {
            ...
            if (payload.action === 'add') {
              ...
              // P0：UI 加条目产生 pending memory_change → 请求 Phase2 整合（idle 即自动进 current）。
              requestPhase2Integrate()
