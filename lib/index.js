// dsh-memory-rollout — Codex-adapted cross-session memory vault for DeepSeek Harness (Host half).
//
...
// 'dsh_rollout' while the npm package is 'dsh-memory-rollout'.
...
export const name = 'dsh-memory-rollout'
...
export async function apply(ctx, config) {
  ...
  ctx.effect(
    ...
    'dsh-memory-rollout.domainClose',
  )
  ...
  // ── runtime config overlay (settings page → dsh-memory-rollout.settings.json) ────
  // The plugin config is resolved from cordis.patch.yml at boot. The settings
  // page edits it at runtime via /dsh-memory-rollout/config, and the changed fields are
  // persisted to a sibling settings file so they survive a restart. This file
  // lives at <ds_home>/dsh-memory-rollout.settings.json (NOT inside memories/, so an
  ...
  const settingsPath = () => path.join(dsHome(), 'dsh-memory-rollout.settings.json')
  const readSettings = () => {
    try {
   ...
   * (no LLM for that route) rather than guessing. This is how dsh-memory-rollout "leaves
  ...
  async function sessionMessagesByPersistence(sessionId) {
    ...
    try {
      ...
      return { events, messages: s.deriveMessages(), cwd: header && header.cwd, sourceStatus: 'ok' }
    } catch {
      ...
      return { events: events || [], messages: [], cwd: header && header.cwd, sourceStatus: 'unavailable' }
    }
  }
  ...
  async function withWrite(fn, opts = {}) {
    if (writeBusy) {
      const e = new Error(
        opts.conflictMessage || '[dsh-memory-rollout] another write is in progress — retry shortly',
      )
      ...
      if (status === 'succeeded_with_output' && extraction) {
        ...
        if (evidence && evidence.content) {
          try {
            ...
            try { console.error('[dsh-memory-rollout] evidence file write failed:', (e && e.message) || e) } catch {}
      ...
      for (;;) {
        ...
        try {
          ...
          if (persisted === null) {
            ...
            status = 'failed_retryable'
            errMsg = `[dsh-memory-rollout] stage1 source unavailable: ${claimed.session_id}`
          ...
          if (!sourceUnavailable && persisted) {
            ...
            if (elig && elig.eligible === false) {
              ...
              errMsg = `[dsh-memory-rollout] skip ${skipReason}`
  ...
  function scheduleStage1Drain() {
    ...
    setImmediate(() => {
      ...
      drainStage1Jobs().catch((err) => {
        try { console.error('[dsh-memory-rollout] stage-1 drain error:', err) } catch {}
      })
    })
  }
  function scheduleStage1Wake(nextAt) {
    ...
    stage1WakeTimer = setTimeout(() => {
      stage1WakeTimer = null
      drainStage1Jobs().catch((err) => {
        try { console.error('[dsh-memory-rollout] stage-1 wake drain error:', err) } catch {}
      })
    }, delay)
  }
  ...
  async function enqueueStage1JobIntoTable(sessionId, watermark, opts = {}) {
    ...
    throw new Error('[dsh-memory-rollout] enqueue failed after retries: ' + String((lastErr && lastErr.message) || lastErr))
  }
  ...
  function requestPhase2Integrate() {
    ...
    phase2AutoTimer = setImmediate(() => {
      ...
      phase2Integrate().catch((err) => {
        try { console.error('[dsh-memory-rollout] phase-2 auto integrate error:', err) } catch {}
      })
    })
  ...
  function schedulePhase2Wake(nextAt) {
    ...
    phase2WakeTimer = setTimeout(() => {
      phase2WakeTimer = null
      phase2Integrate().catch((err) => {
        try { console.error('[dsh-memory-rollout] phase-2 wake drain error:', err) } catch {}
      })
    }, delay)
  }
  ...
  // 活跃表因此不再随历史线性增长。安全边界（设计文档 dsh-memory-rollout-P1归档协议设计-2026-08-29.md）：
    ...
    return withWrite(async () => {
      ...
      for (const [id, p] of publishVersionsTable.entries()) {
        ...
        try {
          ...
          try { console.error('[dsh-memory-rollout] version archive move failed (kept unarchived for retry):', e) } catch {}
        }
        if (!moved) continue
      ...
      for (const [id, p] of publishVersionsTable.entries()) {
        ...
        if (!dryRun) {
          try {
            ...
            try { console.error('[dsh-memory-rollout] version restore move failed:', e) } catch {}
          }
        }
      }
      return report
  ...
  const integrateTool = defineTool({
    ...
    async execute() {
      try {
        ...
        try { console.error('[dsh-memory-rollout] memory_integrate integrate() error:', err) } catch {}
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        if (!enq.queued && enq.error) {
          try { console.error('[dsh-memory-rollout] memory_precompact enqueue returned unqueued:', enq.error) } catch {}
        }
      } catch (err) {
        try { console.error('[dsh-memory-rollout] memory_precompact enqueue error:', err) } catch {}
      }
      scheduleStage1Drain()
      schedulePhase2Wake(nextPhase2WakeAt(Date.now()))
      ...
      try {
        ...
        try { console.error('[dsh-memory-rollout] memory_precompact integrate() error:', err) } catch {}
    ...
    try {
      ...
      if (stage1JobsTable.size > 0) {
        ...
        try { fs.renameSync(oldStage1StatePath(), archiveSkipped) } catch (e2) {
          try { console.error('[dsh-memory-rollout] stage-1 migration: table non-empty, could not archive legacy file (best-effort):', e2) } catch {}
        }
        try { console.error(`[dsh-memory-rollout] stage-1 migration: skipped (stage1_jobs already non-empty), archived legacy file -> ${archiveSkipped}`) } catch {}
        return 0
      }
      ...
      try { console.error(`[dsh-memory-rollout] stage-1 migration: moved legacy state to the storage-domain tables (${n} jobs), archived old file -> ${archive}`) } catch {}
      return n
    } catch (err) {
      try { console.error('[dsh-memory-rollout] stage-1 migration error:', err) } catch {}
      return 0
    }
  }
  ...
  const archiveLegacyPipelineState = () => {
    try {
      ...
      try { console.error(`[dsh-memory-rollout] pipeline-state migration: archived retired state -> ${archive}`) } catch {}
      return 1
    } catch (err) {
      try { console.error('[dsh-memory-rollout] pipeline-state migration error:', err) } catch {}
      return 0
    }
  }
  try {
    ...
    try { console.error('[dsh-memory-rollout] startup pipeline-state migration error:', err) } catch {}
  }
  try {
    await migrateStage1FromFile()
  } catch (err) {
    try { console.error('[dsh-memory-rollout] startup stage-1 migration error:', err) } catch {}
  }
  try {
    await recoverStage1Jobs(Date.now())
  } catch (err) {
    try { console.error('[dsh-memory-rollout] startup stage-1 recover error:', err) } catch {}
  }
  // GPT P1-1：启动修复 forget/supersede 的 entry↔change 半提交（幂等）。
  try {
    await reconcileChangeOutbox()
  } catch (err) {
    try { console.error('[dsh-memory-rollout] startup change-outbox reconcile error:', err) } catch {}
  }
  ...
  try {
    ...
    try { console.error('[dsh-memory-rollout] startup phase-2 binding reconcile error:', err) } catch {}
  }
  ...
  try {
    ...
    try { console.error('[dsh-memory-rollout] startup phase-2 request error:', err) } catch {}
  }
    ...
    ctx.on('session/disposed', async (session) => {
      ...
      if (sid) {
        try {
          ...
          const enq = await enqueueStage1JobIntoTable(sid, contentWatermark(raw))
          if (!enq.queued && enq.error) {
            try { console.error('[dsh-memory-rollout] session/disposed enqueue returned unqueued:', enq.error) } catch {}
          }
        } catch (err) {
          try { console.error('[dsh-memory-rollout] session/disposed enqueue error:', err) } catch {}
        }
      }
  ...
  ctx.effect(
    ...
    'dsh-memory-rollout.eventsDispose',
  )
  ...
  if (webServer !== undefined) {
    ...
    const route = {
      kind: 'exact',
      path: '/dsh-memory-rollout/entries',
    ...
    const overviewRoute = {
      kind: 'exact',
      path: '/dsh-memory-rollout/overview',
      handler: async (req, res) => {
        try {
          const d = dirs()
          const summary = readMemorySummary()
    ...
    // ── /dsh-memory-rollout/config — read + update the plugin config at runtime ─────
    ...
    const configRoute = {
      kind: 'exact',
      path: '/dsh-memory-rollout/config',
      handler: async (req, res) => {
        try {
          if (req.method === 'GET') {
            sendJson(res, 200, {
    ...
    // ── /dsh-memory-rollout/export — package the whole memories/ tree for backup ────
    ...
    const exportRoute = {
      kind: 'exact',
      path: '/dsh-memory-rollout/export',
      handler: (req, res) => {
        try {
          if (req.method !== 'GET') {
            sendJson(res, 405, { error: 'method not allowed' })
          ...
          res.statusCode = 200
          res.setHeader('Content-Type', 'application/json; charset=utf-8')
          res.setHeader('Content-Disposition', `attachment; filename="dsh-memory-rollout-memories-${stamp}.json"`)
          res.end(body)
        } catch (err) {
          sendJson(res, 500, { error: String((err && err.message) || err) })
        }
    ...
    // ── /dsh-memory-rollout/import — restore a backup (back up existing first) ──────
    ...
    async function importBundle(rawText) {
      ...
      return withWrite(() => importBundleUnlocked(rawText), {
        importConflict: true,
        conflictMessage: '[dsh-memory-rollout] another import is already in progress — retry shortly',
      })
    }
    ...
    async function importBundleUnlocked(rawText) {
      ...
      if (!bundle || typeof bundle !== 'object' || bundle.format !== 'dsh-memory-rollout-memory-backup') {
        throw new Error('invalid bundle: not a dsh-memory-rollout memory export')
      }
      ...
      try {
        ...
        try {
          ...
          await writeChangeRecord('import', {
            note: 'imported dsh-memory-rollout memory bundle',
          ...
          try {
            ...
            try {
              console.error('[dsh-memory-rollout] import rollback failed:', rollbackErr)
            } catch {}
            ...
            const f = new Error('[dsh-memory-rollout] ' + manual)
          ...
          const retryable = new Error(
            '[dsh-memory-rollout] import failed, but the previous memory was restored (nothing lost; you can retry). ' +
    ...
    const importRoute = {
      kind: 'exact',
      path: '/dsh-memory-rollout/import',
      handler: async (req, res) => {
        try {
          if (req.method !== 'POST') {
            sendJson(res, 405, { error: 'method not allowed' })
    ...
    ctx.effect(() => () => {
      ...
      disposeExport()
      disposeImport()
    }, 'dsh-memory-rollout.routeDispose')
  }
}
