...
export async function apply(ctx, config) {
  ...
  async function sessionMessagesByPersistence(sessionId) {
    ...
    try {
      ...
      return { events, messages: s.deriveMessages(), cwd: header && header.cwd }
    } catch {
      // M2：即使消息无法重建（corrupt/unreadable），也保留已读到的 events 给资格判定，
      // 避免外部上下文会话在消息重建失败时因 events 被清空而漏判。
      return { events: events || [], messages: [], cwd: header && header.cwd }
    }
  }
    ...
    try {
      ...
      for (;;) {
        ...
        try {
          ...
          // 外部上下文已判不合格 → raw 不受理，也不烧配额。
          if (status !== 'succeeded_no_output' && raw.trim().length >= 60) {
            await withWrite(async () => {
              const m = readStage1Meta()
              if (m.runDay !== dayKey()) {
                m.runDay = dayKey()
          ...
          try {
            // 外部上下文不合格时直接走 no-output，跳过 LLM。
            const out = status === 'succeeded_no_output'
              ? { status: 'succeeded_no_output', extraction: null }
