...
export async function apply(ctx, config) {
  ...
  function writeSessionDraft(sessionId, cwd, title, body) {
    ...
    writeText(file, header + String(body || '').trim() + '\n')
  ...
  function writeExtractedDraft(sessionId, cwd, extraction) {
    ...
    const header = [
      ...
      `updated_at: ${nowIso()}`,
      `cwd: ${cwd || ''}`,
      ...(extraction.slug ? [`slug: ${extraction.slug}`] : []),
      ...(extraction.keywords ? [`keywords: ${extraction.keywords}`] : []),
      '',
      `# 会话草稿 ${extraction.title || ''}`.trimEnd(),
      '',
    ].join('\n')
    const summary = String(extraction.rollout_summary || '').trim()
    const raw = String(extraction.raw_memory || '').trim()
    let body = summary
  ...
  function writeRegistry() {
    ...
    if (entries.length) {
      ...
      for (const e of entries) {
        lines.push(
          `- [${e.tags.join(',')}] ${e.content} (session=${e.sessionId || '-'}, updated=${e.updatedAt})`,
        )
      }
      lines.push('')
    }
  ...
  function writeSummary() {
    ...
    if (prefs.length) for (const e of prefs) lines.push(`- ${e.content}`)
  ...
  function emptyPipelineState() {
    return {
      sessions: {}, // sessionId -> { sessionId, lastActivityAt, summarizedAt }
      global: {
        lastPhase2At: '',
        runsToday: 0,
  ...
  /** Record a session's activity + mark it summarized (called after phase 1). */
  function upsertPipelineSession(state, sessionId, { active = false, summarized = false } = {}) {
  ...
  async function extractWithLlm(raw) {
    ...
    const inputText = truncateTranscript(prompt, config.maxExtractTokens || 8000)
    ...
    return {
      rollout_summary: String(parsed.rollout_summary || '').trim(),
      raw_memory: String(parsed.raw_memory || '').trim(),
      slug: safeSlug(parsed.slug || 'note'),
      keywords: Array.isArray(parsed.keywords)
        ? parsed.keywords.map(String).join(',')
  ...
  async function pipelinePhase1(session) {
    ...
    for (const [key, rec] of Object.entries(state.sessions)) {
      ...
      const draftAge = daysSince(rec && rec.summarizedAt || rec && rec.lastActivityAt)
    ...
    // Draft each collected candidate. Phase 3: the TRIGGER session is refined via
    // ctx.llm into a {raw_memory, rollout_summary, slug} summary; any LLM failure
    // falls back to the literal snapshot. Secondary candidates are also rebuilt
    // from the raw persisted session events (never from the existing draft, which
    // caused nesting) and keep the literal path — no LLM for them, so one run
    // costs at most one extraction call (on the trigger), never startup/per-turn.
    let drafted = 0
    for (const c of candidates) {
      ...
      if (!raw && c.from !== 'trigger') {
        // No new raw content for a secondary candidate: keep the existing draft
        // as-is (do not rewrite → no nesting). Do NOT mark summarized either, so
        // the session can be revisited when its raw log becomes readable.
        continue
      }
      let extraction
      if (c.from === 'trigger') {
        // Only refine a non-trivial transcript — an empty/blurb session is a
        // no-op (saves quota). `extractWithLlm` returns null on any failure.
        extraction = raw && raw.trim().length >= 60 ? await extractWithLlm(raw) : null
        if (!extraction) {
          extraction = {
            rollout_summary: raw,
            raw_memory: raw,
            slug: safeSlug(sid || c.sessionId || 'unknown'),
            keywords: '',
            title: '',
          }
        }
      ...
      upsertPipelineSession(state, c.sessionId, { summarized: true })
      drafted++
    }
  ...
  async function runPipeline(trigger, session) {
    ...
    if (pipelineLock) {
      return { ran: false, reason: 'locked', trigger }
    }
    pipelineLock = true
    try {
      // Daily budget guard.
    ...
    async execute(args) {
      ...
      if (entries.length) {
        const notes = entries.map(
          (e, i) => `${e.sessionId || 'entries'}:${i + 1}|note=[memory recall hit]`,
        )
  ...
  const eventDisposers = [
    // Main trigger hook: the session is being disposed (ended) — run the pipeline.
    ctx.on('session/disposed', (session) => {
      if (config.autoTrigger === 'off') return
      try {
        kickPipeline('sessionEnd', session)
      } catch (err) {
        try {
          console.error('[dsh-rollout] session/disposed handler error:', err)
        } catch {}
      }
    }),
  ...
  if (webServer !== undefined) {
    ...
    async function importBundle(rawText) {
      ...
      const stamp = nowIso().replace(/[:.]/g, '-').slice(0, 19)
      const tmpRoot = path.join(dsHome(), `memories-import-tmp-${stamp}`)
      const backupDir = path.join(dsHome(), `memories-backup-${stamp}`)
      ...
      try {
        ...
        for (const f of files) {
          ...
          seenPaths.add(key)
          fileEntries.push({ rel, content: Buffer.from(String(f.content || ''), 'base64') })
        }
        ...
        // 3) Atomic switch (true replacement, not per-file overwrite): clear the old
        //    tree, copy the temp tree in, then restore the entries table. On ANY
        //    failure roll back to the pre-import state.
        try {
          if (exists(root)) fs.rmSync(root, { recursive: true, force: true })
          fs.mkdirSync(root, { recursive: true })
        ...
        // 4) Regenerate consistency artifacts from the restored files.
        integrate()
        return {
          ok: true,
