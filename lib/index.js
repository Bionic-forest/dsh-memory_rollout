...
export async function apply(ctx, config) {
  ...
  async function reconcilePhase2Bindings(nowMs) {
    return withWrite(async () => {
      ...
      // P1 归档协议：phase2 批次归档后，其绑定的 input 不应被当孤儿解绑（否则重复消费）。
      const jobStatusArchived = new Set()
      for (const [id, j] of phase2JobsArchiveTable.entries()) if (j) jobStatusArchived.set(id, j.status)
      let fixed = 0
      const unbindOrphan = async (table, key, rec) => {
        if (!rec || !rec.phase2_batch_id) return
        const target = jobStatus.get(rec.phase2_batch_id)
        // 只把「指向不存在的批」当孤儿；failed_terminal 批真实存在、且已归档批也存在 → 不解绑（防无限重试/重复消费）。
        const orphan = !target && !jobStatusArchived.has(rec.phase2_batch_id)
        if (orphan) {
  ...
  async function archiveVault(opts = {}) {
    ...
    // 需保留的版本：current + 最近 2 个非当前 published（按 created_at 降序），其余可归档。
    // current 直接读 current.json 的 version id（归档判断不依赖版本目录校验/回退，确定性）。
    let cur = ''
    ...
    return withWrite(async () => {
      ...
      for (const [id, p] of publishVersionsTable.entries()) {
        ...
        try {
          ...
          try { console.error('[dsh-rollout] version archive move failed:', e) } catch {}
        }
  ...
  const archiveVaultTool = defineTool({
    ...
    description:
      '调试/内部：P1 归档协议——默认 dry-run 统计各表可归档量；dryRun=false 时仅归档绝对安全的已消费 memory_changes 到归档表（不硬删、可恢复）。stage1_jobs/outputs、phase2_jobs、versions 因需配套读路径/reconcile 改造而暂缓，本步仅统计并标注边界。',
    parameters: {
      dryRun: { type: 'boolean', default: true, description: 'true=只统计不归档（默认）；false=仅归档绝对安全的 consumed memory_changes。' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render: (_args, value) => [
        { type: 'text', text: `归档${value.dryRun ? '(dry-run)' : ''}：已归档 ${value.archived} 条变更、${value.archivedVersions} 个版本；候选 jobs=${value.candidates.stage1_jobs} outputs=${value.candidates.stage1_outputs} phase2=${value.candidates.phase2_jobs} changes=${value.candidates.changes} versions=${value.candidates.versions}` },
      ],
    },
    async execute(args) {
