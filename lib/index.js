...
export async function apply(ctx, config) {
  ...
  async function pipelinePhase1(session) {
    ...
    // falls back to the literal snapshot. Secondary candidates keep the literal
    // path (already drafted; copy as-is) — no LLM for them, so one run costs at
    // most one extraction call (on the trigger), never startup/per-turn.
    let drafted = 0
    for (const c of candidates) {
      ...
      if (c.from === 'trigger' && session) {
        ...
        raw = readText(
          path.join(
            dirs().summaries,
            `${safeSlug(c.sessionId || 'unknown')}.md`,
          ),
        )
      }
      if (!raw && c.from !== 'trigger') {
        // Secondary candidates with no draft content: mark summarized (already
        // captured) but do not write an empty draft.
  ...
  if (webServer !== undefined) {
    ...
    async function importBundle(rawText) {
      ...
      // 1) Back up the existing memory root first so a failed import loses nothing.
      const root = memoryRoot()
      const stamp = nowIso().replace(/[:.]/g, '-').slice(0, 19)
      const backupDir = path.join(dsHome(), `memories-backup-${stamp}`)
      if (exists(root)) fs.cpSync(root, backupDir, { recursive: true })
      ensureLayout()

      // 2) Write each bundled file to its (sanitized) path under the memory root.
      ...
      for (const f of files) {
        ...
        fs.writeFileSync(target, Buffer.from(String(f.content || ''), 'base64'))
        written++
      }

      // 3) Restore the long-term entries table (clear + re-put).
      let entryCount = 0
      ...
      integrate()
      return { ok: true, fileCount: written, entryCount, backup: path.basename(backupDir) }
    }
