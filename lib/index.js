...
/**
 * 阶段 B：从 stage-1 outputs 里选出「上次成功基线之后」变化/新增的产物，作为
 * Phase 2 整合的增量输入（§7.4：不无差别整库扫描）。`lastSuccessWatermark` 为
 * 上次成功发布的来源基线；等于它的产物视为已整合。纯函数可单测。
 */
export function selectPhase2Inputs(outputs, lastSuccessWatermark) {
  if (!outputs || typeof outputs !== 'object') return []
  return Object.values(outputs).filter((o) => {
    if (!o || typeof o !== 'object') return false
    if (!lastSuccessWatermark) return true
    return String(o.source_watermark) !== String(lastSuccessWatermark)
  })
}

/**
 * 阶段 B：校验整合模型产物的结构与秘密（§7.5 第 5/8 步）。要求：
 *  - memory_summary / registry 非空字符串；
...
export function validatePhase2Output(output, opts = {}) {
  ...
  if (!output || typeof output !== 'object') errors.push('output missing')
    ...
    if (typeof output.memory_summary !== 'string' || !output.memory_summary.trim()) errors.push('memory_summary missing/empty')
    if (typeof output.registry !== 'string' || !output.registry.trim()) errors.push('registry missing/empty')
...
export async function apply(ctx, config) {
  ...
  async function phase2Integrate() {
    return withWrite(async () => {
      ...
      // Publish — validation guarantees no unredacted secret.
      writeText(path.join(memoryRoot(), 'memory_summary.md'), result.memory_summary)
      writeText(path.join(memoryRoot(), 'MEMORY.md'), result.registry)
