 ...
 *   排序影响（如实）：仅当两条候选在相关性与上次使用时间上打平，`usage_count=1` 才压过 `0`。
 */
...
/** 受限会话把结果 JSON 落到记忆根内的子目录（读回后即清理；`write` 在该会话白名单里）。 */
...
/**
 * t213：在受限会话里**真跑一轮**整合（模型调用 + 可能的工具执行都在该会话内）。
 ...
 * @param opts.memoryRoot 记忆根（= 该会话 cwd；产物文件写在其内）。
 * @param opts.batchId 批 id（产物文件名）。
 * @param opts.timeoutMs 等待上界（默认 `EXECUTOR_TURN_TIMEOUT_MS`）。
 */
export async function runConsolidationExecutorTurn({ executor, prompt, systemPrompt, memoryRoot: root, batchId, timeoutMs } = {}) {
  ...
  const outDir = path.join(String(root || ''), EXECUTOR_OUT_SUBDIR)
  const outFile = path.join(outDir, `${String(batchId || 'batch')}.json`)
  try { fs.mkdirSync(outDir, { recursive: true }) } catch { /* 目录建不出不致命：还有事件回读这条退路 */ }
  const before = sessionEventCount(agent)
  const instruction =
    `\n\n## OUTPUT DELIVERY (executor session)\n` +
    `You are running inside a restricted session whose working directory is the memory root.\n` +
    `After you decide the final JSON answer, you MUST ALSO write that exact JSON text to the file:\n` +
    `  ${outFile}\n` +
    `using the \`write\` tool (create parent directories as needed). Write nothing else there.\n`
  ...
  try {
    ...
    return { ok: false, reason: 'executor-dispatch-threw: ' + (err && err.message ? err.message : String(err)), sessionId, activity: executorActivity(agent, before) }
  }
  try {
    await withTimeoutMs(agent.whenIdle(), Number(timeoutMs) > 0 ? Number(timeoutMs) : EXECUTOR_TURN_TIMEOUT_MS)
  } catch (err) {
    const reason = isWriteConflictError(err) ? 'executor-turn-write-conflict' : 'executor-turn-failed: ' + (err && err.message ? err.message : String(err))
    return { ok: false, reason, sessionId, activity: executorActivity(agent, before) }
  }
  let text = ''
  let source = ''
  try {
...
export async function apply(ctx, config) {
  ...
  async function processPhase2Batch(batch) {
    ...
    let executor = null
    let executorObs = { path: '', sessionId: '', restricted: false, reason: '', activity: '', source: '' }
