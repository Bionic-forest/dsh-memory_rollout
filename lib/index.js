...
export async function apply(ctx, config) {
      ...
      for (;;) {
        ...
        try {
          ...
          if (persisted === null) {
            ...
            status = 'failed_retryable'
            errMsg = `[dsh-rollout] stage1 source unavailable: ${claimed.session_id}`
          ...
          if (!sourceUnavailable && persisted) {
            ...
            if (elig && elig.eligible === false) {
              ...
              errMsg = `[dsh-rollout] skip ${skipReason}`
  ...
  function requestPhase2Integrate() {
    ...
    phase2AutoTimer = setImmediate(() => {
      ...
      phase2Integrate().catch((err) => {
        try { console.error('[dsh-rollout] phase-2 auto integrate error:', err) } catch {}
      })
    })
  ...
  function memoryCitationEntries(entries) {
    ...
    return entries.map((e) => {
      ...
      if (e.sessionId) {
        ...
        // P1-5 / P0-R2-2 边界：这是**会话级草稿指针**（entry 经 sessionId 绑定到该会话，草稿即该会话
        // 记录），因此只要求「文件存在 + 行段有效」（validateSourceRef 不传 content）+ 与 entry 共享
        // 至少一个特征词，并把 `1-N`（整个草稿正文）当作粗粒度来源指针。P0-R2-2 的「规范化完整子串」
        // 严格要求**只用在 Stage1 证据路径**（sourceRefForEntry 传 opts.content），防止「同主题词、
        // 不同事实」的无关 entry 误引用**精确证据行段**；草稿回退不是该场景，需保留宽松关联才不误杀。
        const draft = validateSourceRef({ path: relPath, startLine: 1, endLine: 0 }, memoryRoot())
        if (draft.ok) {
          const span = readText(path.join(memoryRoot(), relPath))
          const spanSet = new Set(tokenizeContent(span))
          const contentTokens = tokenizeContent(String(e.content || ''))
          if (contentTokens.some((t) => spanSet.has(t))) return memo(relPath, 1, draft.spanEnd)
        }
      }
  ...
  try {
    ...
    try { console.error('[dsh-rollout] startup phase-2 request error:', err) } catch {}
  }
