...
export async function apply(ctx, config) {
  ...
  async function drainStage1Jobs() {
    return withWrite(async () => {
      ...
      for (;;) {
        const now = Date.now()
        const job = claimStage1Job(state, now, leaseMs, owner)
        if (!job) break
        try {
          let raw = ''
          const persisted = job.session_id ? await sessionMessagesByPersistence(job.session_id) : null
          if (persisted && Array.isArray(persisted.messages) && persisted.messages.length) {
            raw = messagesToDraftBody(persisted.messages)
          }
          const out = await extractWithOutcome(raw)
          if (out.status === 'succeeded_with_output' && out.extraction) {
            stage1FinishJob(state, job, 'succeeded_with_output', { output: out.extraction }, new Date(now))
          } else if (out.status === 'succeeded_no_output') {
            stage1FinishJob(state, job, 'succeeded_no_output', {}, new Date(now))
          } else {
            stage1FinishJob(state, job, 'failed_retryable', { error_message: 'extraction failed' }, new Date(now))
          }
        } catch (err) {
          stage1FinishJob(state, job, 'failed_retryable', { error_message: String((err && err.message) || err) }, new Date(now))
        }
        processed++
      }
  ...
  const eventDisposers = [
    // Main trigger hook: the session is being disposed (ended). 阶段 A: the event
    // handler is SHORT — it only enqueues a persistent stage-1 job (event-only, no
    // model call) and kicks the existing pipeline (kept for backwards compatibility
    // during the phase-A migration). watermark = content fingerprint of the session.
