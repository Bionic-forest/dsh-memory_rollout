...
export async function apply(ctx, config) {
   ...
   * 纯内容构建（不读盘/不写盘），可被 stage-1 提炼成功路径与旧 pipelinePhase1 复用。
   */
  ...
  const loadStage1State = () => {
    ...
    const outputs = {}
    for (const [k, v] of stage1OutputsTable.entries()) outputs[k] = v
    return { jobs, outputs, global: readStage1Meta() }
  }
  const saveStage1State = async (state) => {
    const jobs = state.jobs || {}
    const outputs = state.outputs || {}
    await Promise.all([
      ...Object.keys(jobs).map((k) => stage1JobsTable.put(k, jobs[k])),
      ...Object.keys(outputs).map((k) => stage1OutputsTable.put(k, outputs[k])),
      writeStage1Meta(state.global || {}),
    ])
  }
  ...
  async function enqueueStage1JobIntoTable(sessionId, watermark, opts = {}) {
    ...
    // 锁忙重试耗尽：绝不静默丢——记录明确报错日志（§4 P0-2）。待第 4 步接入收件箱兜底。
    try { console.error('[dsh-rollout] session/disposed enqueue failed after retries:', lastErr) } catch {}
  ...
  // isReasoningEffortError / redactSecrets / nowIso / loadStage1State / saveStage1State
  ...
  /** Assemble the consolidation prompt from incremental inputs + current files. */
  function buildConsolidationPrompt(inputs, currentSummary, currentRegistry) {
    const lines = []
  ...
  /**
   * 阶段 B：原子发布 Phase 2 的一对文件（memory_summary.md + MEMORY.md）。每份先写
   * 到同目录 `.tmp` 再 `renameSync` 覆盖最终文件；任一步失败就清理两个 tmp，把旧版
   * 完整保留并抛错（调用方不推进水印）。比「直接写最终文件」强：第二个文件写失败时
   * 不会留下「总纲新版、注册表旧版」的半状态（M2 / §5.3 per-file atomic + 两文件先备后切）。
   */
  function atomicWritePair(summaryContent, registryContent) {
    const summaryPath = path.join(memoryRoot(), 'memory_summary.md')
    const registryPath = path.join(memoryRoot(), 'MEMORY.md')
    const sTmp = summaryPath + '.tmp'
    const rTmp = registryPath + '.tmp'
    try {
      writeText(sTmp, summaryContent)
      writeText(rTmp, registryContent)
  ...
  /**
   * 阶段 B：Phase 2 全局整合（真整合）。
   * 触发：有新 Phase 1 产物（stage-1 outputs 有增量）或手动。用 selectPhase2Inputs 选增量；
   * 无增量 → 不调用模型，返回 { ran:false, reason:'no-change' }。
   * 有增量 → 调 consolidation LLM 产出 { memory_summary, registry }，validatePhase2Output 校验：
   *   ok → 发布（写 memory_summary.md + MEMORY.md）+ 推进 lastSuccessWatermark 为最新输入的
   *         source_watermark + lastPhase2At=now；fail → 保留旧版不写，记 phase2_last_error。
   * 发布/推进水印/写 .phase2-authoritative 走 withWrite 全局互斥；但「读输入 + 构造 prompt +
   * consolidateWithLlm 的模型调用」放到锁外（M4）——模型期间不长期持有全局写锁，避免阻塞
   * 导入/UI/阶段 A 提交；发布前在锁内重新校验（防止期间状态变化）。
   * 返回 { ran:true, ok:true, watermarks } / { ran:true, ok:false, errors } / { ran:false, reason }。
   */
  async function phase2Integrate() {
    // ── M4: 锁外只读选输入 + 构造 prompt（读 stage-1 outputs 与当前文件，无写副作用）──
    const state0 = loadStage1State()
    const base0 = state0.global && state0.global.lastSuccessWatermark
    const inputs = selectPhase2Inputs(state0.outputs, base0)
    if (!inputs.length) {
      return { ran: false, reason: 'no-change' }
    }
    const prompt = buildConsolidationPrompt(
      inputs,
      readText(path.join(memoryRoot(), 'memory_summary.md')),
      readText(path.join(memoryRoot(), 'MEMORY.md')),
    )
    // 慢速模型调用在写锁外执行（M4）。
    const result = await consolidateWithLlm(prompt)
    // ── 发布 / 推进在写锁内，重新读状态防止期间变化 ──
    return withWrite(async () => {
      const state = loadStage1State()
      const base = state.global && state.global.lastSuccessWatermark
      const finalInputs = selectPhase2Inputs(state.outputs, base)
      if (!result) {
        // LLM unavailable / unrouteable / failed: do NOT publish, do NOT advance.
        const errors = ['llm-unavailable']
        state.global.phase2_last_error = errors.join('; ')
        await saveStage1State(state)
        return { ran: true, ok: false, errors }
      }
      if (!finalInputs.length) {
        // 模型调用期间这些输入已被并发消费（race）——什么都不发布。
        state.global.phase2_last_error = 'concurrent consumption; nothing to publish'
        await saveStage1State(state)
        return { ran: false, reason: 'no-change' }
      }
      const validation = validatePhase2Output(result)
      if (!validation.ok) {
        // Keep the old version: do NOT write. Record the error (retryable later).
        state.global.phase2_last_error = validation.errors.join('; ')
        await saveStage1State(state)
        return { ran: true, ok: false, errors: validation.errors }
      }
      // Publish — validation guarantees no unredacted secret. M2: atomic pair
      // (write both .tmp, then rename), so a second-file failure keeps the old
      // version intact and never advances the watermark.
      try {
        atomicWritePair(result.memory_summary, result.registry)
      } catch (e) {
        const errors = ['publish-failed: ' + String((e && e.message) || e)]
        state.global.phase2_last_error = errors.join('; ')
        await saveStage1State(state)
        return { ran: true, ok: false, errors }
      }
      // Advance the success watermark to the LATEST input's source_watermark.
      const newest = pickNewestInput(finalInputs)
      state.global.lastSuccessWatermark = newest ? String(newest.source_watermark) : ''
      state.global.lastPhase2At = nowIso()
      state.global.phase2_last_error = ''
      // M1: mark the consumed inputs as `selected_for_phase2` so a later pass
      // never re-selects them (durable per-output consumption marker).
      for (const it of finalInputs) {
        if (it && typeof it === 'object') it.selected_for_phase2 = true
      }
      await saveStage1State(state)
      // H3b: the published pair is now the authoritative (LLM-consolidated) version.
      // A later deterministic integrate() must not clobber it (it checks this marker).
      try { writeText(path.join(memoryRoot(), '.phase2-authoritative'), '1') } catch {}
      return {
        ran: true,
        ok: true,
        watermarks: finalInputs.map((it) => String(it.source_watermark)),
      }
    })
  }
  ...
  const phase2IntegrateTool = defineTool({
    ...
    description: '调试/内部：手动触发一次 Phase 2 全局整合（用 consolidation LLM 把 stage-1 的增量产物整合成新的 memory_summary.md + MEMORY.md，校验通过后发布）。与阶段 A 的 memory__stage1_drain 一致，供测试/手动触发；不改变现有 auto 管线。',
    parameters: {},
    output: {
      schema: {
        type: 'object',
  ...
  const draftTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      // 不调用任何加锁函数，不会嵌套死锁）。
      const file = await withWrite(() => writeSessionDraft(sid, cwd, args.title, body))
  ...
  const noteTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      await withWrite(() => writeText(file, header + content + '\n'))
  ...
  const migrateStage1FromFile = async () => {
    try {
      ...
      if (stage1JobsTable.size > 0) return 0
