...
export async function apply(ctx, config) {
  ...
  function searchMemoryFiles(terms, limit) {
    ...
    const scanLines = (text, label, prefix = '') => {
    ...
    // 最相关的 1-2 个草稿/证据（rollout_summaries/*.md）：每个文件取第一个命中行，最多 2 个。
    const summariesDir = dirs().summaries
    let draftPicked = 0
    for (const name of listFiles(summariesDir)) {
      if (!name.endsWith('.md')) continue
      const fp = path.join(summariesDir, name)
      ...
      for (let i = 0; i < lines.length; i++) {
        const t = String(lines[i] || '').trim()
        if (!t || t.length < 6) continue
        if (terms.some((term) => t.toLowerCase().includes(term))) {
          pickedLine = { content: t, citation: `rollout_summaries/${name}:${i + 1}-${i + 1}`, m: 1 }
          break
        }
      }
      if (pickedLine) { out.push(pickedLine); draftPicked++ }
      if (draftPicked >= 2) break
    }
  ...
  const recallTool = defineTool({
    ...
    async execute(args) {
      ...
      // M1：也搜当前记忆文件里的「自动记忆」（Phase1/Phase2 产物归档处），并与 entries 去重。
      const memories = searchMemoryFiles(terms, limit)
        .filter((m) => {
          const mc = normalizeContent(m.content)
          // M1 轻量重复抑制：若记忆文件行与某显式条目归属同一事实（归一化后互为子串），
          // 跳过自动记忆行（显式 entry 已代表它），避免同一记忆重复出现多次。
          return !entries.some((e) => {
            const ec = normalizeContent(e.content)
            return (mc.length > 0 && ec.length > 0) && (mc.includes(ec) || ec.includes(mc))
          })
        })
      ...
      return { entries, memories, count: entries.length, maybeStale, citation }
    },
  })
