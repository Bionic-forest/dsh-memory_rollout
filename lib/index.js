...
export function restrictableGlobalTools(tools, scope) {
  ...
  try {
    ...
    if (set && typeof set[Symbol.iterator] === 'function') {
      out.names = [...set].map(String).filter((n) => n && n !== 'run_code').sort()
      if (out.names.length) out.source = 'view.restrictableNames'
    }
  } catch { /* 落到下面的探测退路 */ }
  if (!out.names.length) {
    // 退路：用一次**注定失败**的探测让宿主把 known 名单回吐（抛点在 append 之前，无副作用）。
    try {
      tools.restrict({ deny: ['\u0000__probe__'] })
    } catch (err) {
      const m = String((err && err.message) || '').match(/known global tools:\s*([\s\S]+)$/)
      if (m) {
        out.names = m[1].split(',').map((s) => s.trim()).filter((n) => n && n !== '(none)' && n !== 'run_code').sort()
        if (out.names.length) out.source = 'restrict-error-known-list'
      }
    }
  }
  out.unknownDesired = wanted.filter((n) => !out.names.includes(n))
  if (!out.names.length) out.note = 'restrictable-name-set-unavailable'
  return out
}
/**
...
export const QUOTA_SEMANTICS = Object.freeze({
  ...
  phase2CallBudget: {
    ...
    debitedBy: [],
    enforcedAt: ['门二只**读** Stage 1 的计数器做判断，自己不记账'],
    isProviderQuota: false,
    note: '因此"保底额度"只能约束 Stage 1 的开销，**不能**约束 Phase 2 的实际消耗——如实标注，不改门逻辑（② 已验证行为）。',
  },
  providerRateLimit: {
...
export function buildReferenceMap(opts = {}) {
  ...
  const collected = []
  const seenPublic = new Set(), seenName = new Set(), seenSession = new Set()
  ...
  const addEntry = (sid, meta = {}) => {
    ...
    if (seenPublic.has(publicPath)) { if (meta.slug) aliasIndex.set(String(meta.slug), publicPath); return null }
  ...
  // ①-1 本批输入（kind=input）：来源身份 = session_id，源版本 = source_watermark。
  ...
  return { entries, byCode, byPublic, byName, bySession, byAlias, legacyNames, unverified, slugSessions, memoryRoot }
}
...
export function extractReferences(text, map) {
  ...
  while ((p = rp.exec(s)) !== null) {
    ...
    // 查找顺序：精确公开路径 → 精确文件名 → **插件记录里的 slug 别名**（找回来源，不猜测）。
    const entry = (map.byPublic && map.byPublic.get(key)) || (map.byName && map.byName.get(name)) || (map.byAlias && map.byAlias.get(bare)) || null
    if (entry) push('path', p[0], p.index, p.index + p[0].length, entry, refLineRange(p[2], p[3]))
 ...
 */
export function renderPhase2References(text, map) {
  const s = String(text == null ? '' : text)
  const { tokens, unmapped, unverified } = extractReferences(s, map)
  const edits = []
  const used = []
  for (const t of tokens) {
    if (t.inCredentialField) continue
    const suffix = t.lineRange ? `:${t.lineRange.start}-${t.lineRange.end}` : ''
    edits.push({ start: t.start, end: t.end, text: t.entry.publicPath + suffix })
  ...
  out += s.slice(pos)
  return { text: out, used: [...new Set(used)], unmapped, unverified }
}
 ...
 */
export async function startConsolidationExecutor({ ctx, memoryRoot: root, sessionId, candidateDir } = {}) {
  ...
  const handle = await agents.create({
    ...
    setup: (childCtx) => {
      ...
      try {
        ...
        const derived = restrictableGlobalTools(tools, childCtx && childCtx.agent)
        restrictObs = derived
  ...
  if (restricted !== true || !policiesApplied) {
    // 边界 1（R2 §8-1）：限制未建立 ⇒ **不走受限路径**（显式回落 + 写明原因），并停掉刚建的会话。
    ...
    return {
      ...
      spec, candidateDir: spec.cwd, handle: null, restricted, restrictCalls, restrictError, applied, stopped, restrictObs,
    }
  }
  return { ok: true, reason: '', spec, candidateDir: spec.cwd, handle, restricted, restrictCalls, restrictError, applied, restrictObs }
}
...
/** 构造发给受限会话的**用户消息**（形状对齐 `@deepseek-ai/dsh-llm` 的 `UserMessage`：`{role,content,source}`）。纯函数，可单测。 */
export function buildExecutorUserMessage(text) {
  return {
    role: 'user',
...
/** 会话事件条数（best-effort；拿不到返回 -1，仅作"会话活动"证据）。 */
function sessionEventCount(agent) {
  try {
    const s = agent && agent.session
    if (!s) return -1
    const evs = typeof s.events === 'function' ? s.events() : s.events
    if (!evs) return -1
    if (typeof evs.length === 'number') return Number(evs.length)
    if (typeof evs[Symbol.iterator] === 'function') return [...evs].length
  } catch {}
  return -1
}

/** 会话活动证据串（不依赖控制台；落批记录用）。 */
function executorActivity(agent, before) {
  const after = sessionEventCount(agent)
  const turns = (() => {
    try {
      const st = agent && agent.status
      if (st && typeof st === 'object' && Number.isFinite(Number(st.turns))) return Number(st.turns)
    } catch {}
    return null
  })()
  const parts = [`events=${before}->${after}`]
...
export async function runConsolidationExecutorTurn({ executor, prompt, systemPrompt, memoryRoot: root, batchId, timeoutMs, candidateDir, attemptTag } = {}) {
  ...
  try {
    ...
    const stopped = await stopConsolidationExecutor(executor, 'executor-dispatch-threw')
    cleanup()
    return { ok: false, reason: 'executor-dispatch-threw: ' + (err && err.message ? err.message : String(err)), sessionId, activity: executorActivity(agent, before), stopped }
  }
  try {
    ...
    const stopped = await stopConsolidationExecutor(executor, base)
    cleanup()
    return { ok: false, reason: base, sessionId, activity: executorActivity(agent, before), stopped }
  }
  const activity = executorActivity(agent, before)
  // 边界 2：**先查权威面，再谈采纳**。
  const changed = authoritativeDiff(authBefore, authoritativeSnapshot(memRoot))
  if (changed.length) {
    const stopped = await stopConsolidationExecutor(executor, 'executor-boundary-violation')
    cleanup()
    return {
      ok: false,
      reason: 'executor-boundary-violation: authoritative surface modified during executor turn: ' + changed.join(', '),
      sessionId, activity, boundaryViolation: true, changed, stopped,
    }
  }
  let text = ''
  let source = ''
  ...
  if (staleNote) {
    cleanup()
    return { ok: false, reason: 'executor-stale-output: ' + staleNote, sessionId, activity }
  }
  if (!text) {
  ...
  cleanup()
  if (!text.trim()) return { ok: false, reason: 'executor-no-output', sessionId, activity }
  return { ok: true, reason: '', text, source, sessionId, activity }
}
...
export async function apply(ctx, config) {
  ...
  function scheduleStage1Wake(nextAt) {
    ...
    stage1WakeTimer = setTimeout(() => {
      ...
      runScheduledPass('stage-1 wake drain error', 'stage1-wake', () => drainStage1Jobs({ budget: perPassSourceBudget() }))
    }, delay)
  }
  ...
  async function processPhase2Batch(batch) {
    ...
    let executor = null
    let executorObs = { path: '', sessionId: '', restricted: false, reason: '', activity: '', source: '', cwd: '', boundaryViolation: '' }
    ...
    if (config.consolidationExecutor !== false) {
      try {
        executor = await startConsolidationExecutor({ ctx, memoryRoot: memoryRoot(), sessionId: executorSessionId, candidateDir: executorWorkspace })
      ...
      executorObs.reason = 'executor-disabled-by-config'
    }
    ...
    try {
      ...
      if (turn && turn.ok) {
        ...
        // t220（R2 §8-2）：执行者**动了权威面** ⇒ 这一轮的产物一律**不采纳**（拒收），落观测后走显式回落。
        //   为什么是"拒收 + 回落"而不是"整批失败"：权威面变化也可能来自别处的并发写（执行者轮次在写锁外），
        //   一律判批失败会重新制造"D1 式"的死循环（重试到上界→abandoned）；而**权威发布始终由外层掌握**，
        //   外层用自己的结果重发一版即可 —— 越界写**不会被采纳进权威状态**。
        if (turn.boundaryViolation) {
          executorObs.boundaryViolation = String((turn.changed || []).join(',') || 'unknown')
          console.warn(`[dsh-memory_rollout] executor boundary violation (authoritative surface modified): ${executorObs.boundaryViolation} — executor result REFUSED, falling back to in-process single-shot`)
        }
      }
      if (!result) {
        executorObs.path = 'in-process-fallback'
        if (!executorObs.reason) executorObs.reason = 'executor-unavailable'
    ...
    const renderedSummary = renderPhase2References(String(result.memory_summary || ''), referenceMap)
    const renderedRegistry = renderPhase2References(String(result.registry || ''), referenceMap)
  ...
  const eventDisposers = [
    ...
    ctx.on('session/disposed', async (session) => {
      ...
      // t210：事件趟**也带 per-pass 来源上限**（codex per pass 语义）；剩余来源交给唤醒趟（30s 间隔）。
      scheduleStage1Drain(perPassSourceBudget())
    }),
