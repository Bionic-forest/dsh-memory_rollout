...
export async function apply(ctx, config) {
   ...
   * 整体用 withWrite 全局互斥（Phase 2 是全局单例作业，与导入/阶段 A 写互斥）。
  ...
  async function phase2Integrate() {
    return withWrite(async () => {
      ...
      const inputs = selectPhase2Inputs(state.outputs, base)
      if (!inputs.length) {
      ...
      const newest = pickNewestInput(inputs)
      ...
      return {
        ran: true,
        ok: true,
        watermarks: inputs.map((it) => String(it.source_watermark)),
      }
    })
  }
