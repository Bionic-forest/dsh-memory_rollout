...
export async function apply(ctx, config) {
  ...
  async function drainStage1Jobs() {
    ...
    const leaseMs = 60000
    const cap = Math.max(1, config.maxModelAttemptsPerDay || 24)
    // Today's model-attempt budget, carried in .stage1-state.json global and
    // persisted on each submit. Enforced so a runaway drain never burns quota
    // (GPT §14.2: 实际尝试数 ≤ N).
    let budgetUsed = (loadStage1State().global && loadStage1State().global.modelAttemptsToday) || 0
    let processed = 0
    let anyOutput = false
    for (;;) {
      if (budgetUsed >= cap) break
      ...
      try {
        ...
        // Count a real model attempt (>=60 chars reaches the LLM); short/no
        // transcript never burns quota (matches extractWithOutcome's gate).
        if (raw.trim().length >= 60) budgetUsed++
        const out = await extractWithOutcome(raw)
        status = out.status
      ...
      await withWrite(() => {
        ...
        const now = new Date()
        state.global.modelAttemptsToday = budgetUsed
        if (status === 'succeeded_with_output' && extraction) {
          anyOutput = true
    ...
    // H3: when this drain produced at least one new incremental output, auto-trigger
    // a real Phase 2 consolidation (write is mutually exclusive via withWrite inside
    // phase2Integrate). Best-effort: a failed/unavailable consolidation is recorded
    // (phase2_last_error) and never throws the drain or breaks the caller.
    if (anyOutput) {
  ...
  // DSH 在提炼中崩溃/重启后，磁盘遗留的过期 `running` 作业必须被回收为 `pending`
  // 并由 drain 消费，否则 `claimStage1Job`（只挑 pending）永远不会领到它们 → 丢任务。
  // 因此 apply() 启动路径：先 stage1Recover（回收过期 running→pending），再
  // setImmediate 排程 drainStage1Jobs 消费。best-effort：fs/io 失败不阻塞插件启动。
  try {
    stage1Recover(path.join(memoryRoot(), '.stage1-state.json'))
  } catch (err) {
    ...
    ctx.on('session/disposed', async (session) => {
      ...
      if (sid) {
        try {
          ...
          // M5：写入 .stage1-state.json 必须过全局写协调。enqueueStage1JobFile 是
          // 模块级同步函数，包 withWrite 即可（withWrite 是 async，须 await）。
          // 持锁时（如导入/phase2/drain 进行中）会抛冲突，被下列 catch 捕获并记录。
          await withWrite(() =>
