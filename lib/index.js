...
export function buildReferenceMap(opts = {}) {
  ...
  const collected = []
  const seenPublic = new Set(), seenName = new Set(), seenAlias = new Set(), seenSession = new Set()
  const addEntry = (sid, meta = {}) => {
    const id = String(sid || '').trim()
    if (!id) return null
    const relPath = `rollout_summaries/${safeSlug(id)}.md`
    const publicPath = renderReferencePath(relPath)
    if (seenPublic.has(publicPath)) { if (meta.slug) seenAlias.add(String(meta.slug)); return null }
    ...
    seenName.add(e.name)
    seenSession.add(id)
    if (e.slug) seenAlias.add(e.slug)
    return e
  }
  ...
  for (const name of legacyNames) {
    ...
    if (seenName.has(name) || seenAlias.has(bare)) continue
...
export async function startConsolidationExecutor({ ctx, memoryRoot: root, sessionId, candidateDir } = {}) {
  ...
  if (restricted !== true || !policiesApplied) {
    ...
    return {
      ...
      spec, candidateDir: spec.cwd, handle: null, restricted, restrictCalls, restrictError, applied, stopped,
    }
  }
  return { ok: true, reason: '', spec, candidateDir: spec.cwd, handle, restricted, restrictCalls, restrictError, applied }
}
...
export async function apply(ctx, config) {
  ...
  async function processPhase2Batch(batch) {
    ...
    // t220（R2 §8-3）：每次尝试一个**独立候选工作区**（`attempt-<attempt_count>-<nonce>`）⇒ 重试不会共用
    //   输出路径、也就读不到上一次尝试的旧结果；子目录再按 attempt 分（见 runConsolidationExecutorTurn）。
    const executorAttemptTag = `${Math.max(0, Number(batch.attempt_count) || 0)}-${crypto.randomBytes(3).toString('hex')}`
    const executorCandidateDir = path.join(memoryRoot(), EXECUTOR_OUT_SUBDIR, EXECUTOR_WORKSPACE_DIRNAME, `attempt-${executorAttemptTag}`)
    if (config.consolidationExecutor !== false) {
      try {
        executor = await startConsolidationExecutor({ ctx, memoryRoot: memoryRoot(), sessionId: `p2-exec-${batch.id}`, candidateDir: executorCandidateDir })
        executorObs.cwd = String((executor && executor.candidateDir) || '')
        if (executor.ok) {
          const ap = executor.applied || {}
          executorObs.sessionId = `p2-exec-${batch.id}`
    ...
    try {
      ...
      if (executor && executor.ok) {
        turn = await runConsolidationExecutorTurn({
          ...
          memoryRoot: memoryRoot(),
          batchId: batch.id,
          candidateDir: executorCandidateDir,
